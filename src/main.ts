...
class TestReporter {
  ...
  readonly failOnNoResults =
    core.getInput("fail-on-no-results", { required: true }) === "true";
  ...
  async run(): Promise<void> {
    ...
    const pattern = this.pathReplaceBackslashes
      ? pathsList.map(normalizeFilePath)
      : pathsList;
    ...
    if (this.failOnError && isFailed) {
      core.setFailed(
        `Failed test were found and 'fail-on-error' option is set to ${this.failOnError}`
      );
      return;
    }
  ...
  async createReport(
    ...
    if (prs.length > 0) {
      ...
      // test
      const allcomments = this.octokit.rest.issues.listComments({
        owner: github.context.repo.owner,
        repo: github.context.repo.repo,
  ...
  getParser(reporter: string, options: ParseOptions): TestParser {
    switch (reporter) {
      ...
      default:
        throw new Error(
          `Input variable 'reporter' is set to invalid value '${reporter}'`
        );
    }
  }
}
